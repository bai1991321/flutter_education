...
import 'package:education_app/ui/fragments_for_student/dashboardFragmentStudent.dart';
import 'package:education_app/ui/fragments_for_student/feedsFragmentStudent.dart';
import 'package:education_app/ui/fragments_for_student/homeFragmentStudent.dart';
import 'package:education_app/ui/fragments_for_student/profileFragmentStudent.dart';
...
class DrawersWidgetStudent extends StatefulWidget {
  DrawersWidgetStudent(
      {Key key,
      this.auth,
      this.facebookAuth,
      this.googleAuth,
  ...
  final drawerItemsStudent = [
    ...
    new DrawerItem("Student Dashboard", Icons.table_chart),
    new DrawerItem("Feeds", Icons.rss_feed),
    new DrawerItem("Profile", Icons.person)
  ];

  @override
  _DrawersWidgetStudentState createState() => _DrawersWidgetStudentState();
}

class _DrawersWidgetStudentState extends State<DrawersWidgetStudent> {
  ...
  Future<List<SchoolData>> _loadSchoolData() async {
    if (listSchoolData != null && listSchoolData.length > 0) {
      return listSchoolData;
    } else {
      String jsonSchoolData = await _loadFromAsset();
      return schoolDataList(jsonSchoolData);
    }
  }

  Future<String> _loadFromAsset() async {
    return await rootBundle.loadString('assets/schooldata.json');
  }

  Future<List<SchoolData>> schoolDataList(String jsonString) async {
    List<dynamic> decoded = await jsonDecode(jsonString);
    listSchoolData = new List<SchoolData>();
    for (var word in decoded) {
      SchoolData schoolData = new SchoolData(word['School_Name'],
          word['Address'], word['City'], word['State'], word['Zip']);
      listSchoolData.add(schoolData);
    }
    return listSchoolData;
  }
  ...
  _getDrawerItemWidget(BuildContext context, int pos) {
    ...
    switch (pos) {
      case 0:
        return new HomeFragmentStudent(openSelectedItem: _openSelectedItem);
      case 1:
        return new DashboardFragmentStudent();
      case 2:
        return new FeedsFragmentStudent();
      case 3:
        return new ProfileFragmentStudent(
            userData: _userData,
            userId: widget.userId,
            storage: widget.storage,
            listSchoolData: this.listSchoolData);
      default:
